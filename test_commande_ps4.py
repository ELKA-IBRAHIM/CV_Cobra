...
# Plage du joystick [-50%,50%] de la vitesse 
...
class MyController(Controller):
    ...
    def on_triangle_press(self): #ouverture pince 
        mcc['p'].cmd_vit_pourcent(50) #à 30% de la vitesse max
    ...
    def on_R1_press(self,value): #déportation droite
        if value<0 :
            value = 0
        value = value*5/3200
        if value >50 :
    ...
    def on_R1_release(self,value): #arret déportation droite
        brushless["av"].cmd_vit_pourcent(0)
        brushless["ar"].cmd_vit_pourcent(0)

    def on_L2_press(self,value): #déportation gauche
        if value>0 :
            value = 0
        value = value*5/3200
        if value <-50 :
    ...
    def on_L2_release(self): #arret déportation gauche
        brushless["av"].cmd_vit_pourcent(0)
        brushless["ar"].cmd_vit_pourcent(0)
